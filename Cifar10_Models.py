...
class SmallDenseNet(nn.Module):
    def __init__(self, num_classes=10):
        ...
        for i, num_layers in enumerate(block_config):
            ...
            ):  # do not add transition layer after the last block
                trans = _Transition(
                    num_input_features=num_features,
...
class LeNet5_BN(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.conv1 = nn.Conv2d(
            3, 48, kernel_size=5, stride=1, padding=2
        )  # Adjusted to match AlexNetSurrogate
        self.bn1 = nn.BatchNorm2d(48)
        self.lrn = nn.LocalResponseNorm(size=2)  # Adding LRN to mimic AlexNetSurrogate
        self.pool1 = nn.MaxPool2d(
            kernel_size=3, stride=2, padding=1
        )  # Adjusted pooling to match

        self.conv2 = nn.Conv2d(
            48, 128, 5, padding=2
        )  # Adjusted to increase filter count
        self.bn2 = nn.BatchNorm2d(128)
        self.pool2 = nn.MaxPool2d(2, 2)

        self.conv3 = nn.Conv2d(
            128, 256, 3, padding=1
        )  # Additional conv layer to increase depth
        self.bn3 = nn.BatchNorm2d(256)
        self.pool3 = nn.MaxPool2d(2, 2)

        self.fc1 = nn.Linear(
            256 * 4 * 4, 512
        )  # Adjusted to account for additional conv layer and increased depth
            ...
            for data in testloader:
                ...
                if criterion.__class__.__name__ == "MSELoss":
                    ...
                    one_hot_encoded_tensor = [
                        one_hot_encode(x.item(), 10) for x in labels
                    ]
                    one_hot_encoded_tensor = torch.tensor(one_hot_encoded_tensor)
                    loss = criterion(outputs, one_hot_encoded_tensor)
                else:
