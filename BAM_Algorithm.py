...
class BasicModelGeneticAlgorithm:
    ...
    def run_genetic_algorithm(self, generations: int, k: int, epsilon: float, population_size: int,
                              search_spread: float) -> list:
        """
        ...
        for generation in range(generations):
            ...
            new_population = self.create_new_generation_with_noise(top_k_population, population, population_size,
                                                                   search_spread)
    ...
    def fitness(self, population: list[tuple[torch.Tensor, torch.Tensor]], epsilon: float) -> list[float]:
        """
    ...
    def select(self, population: list, fitnesses: list[float], k: int) -> tuple[list, list[tuple]]:
        """
    ...
    def predict_and_create_proxy_dataset(self, population: list[torch.Tensor], cur_generation: int, generations: int) -> \
        ...
        for i in range(0, num_samples, batch_size):
            ...
            batch_prediction = self.model(batch).detach()  # Detach predictions to prevent gradient tracking
        ...
        self.query_counter += model_prediction.shape[0]  # Update query counter with the number of samples processed
    ...
    def create_new_generation_with_noise(self, top_k_population: list[torch.Tensor], population: list[torch.Tensor],
                                         population_size: int, search_spread: float) -> list[torch.Tensor]:
        """
        ...
        for i in range(factor - 1):
            ...
            noise = random_number * (
                    noise_size * ss_vector * torch.rand(population_original_shape) - 0.5 * noise_size * ss_vector)
